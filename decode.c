 ...
 */
void printStreamInformation(const AVCodec *codec,
  ...
  fprintf(stderr, "Float Output:  %7s\n",
          !RAW_OUT_ON_PLANAR || av_sample_fmt_is_planar(codecCtx->sample_fmt)
              ? "yes"
              : "no");
}
...
int main(int argc, char *argv[]) {
  ...
  // Initialize the libavformat. This registers all muxers, demuxers and
  // protocols.
  av_register_all();
  ...
  // In case the file had no header, read some frames and find out which format
  // and codecs are used. This does not consume any data. Any read packets are
  ...
  if ((err = avcodec_parameters_to_context(
           codecCtx, formatCtx->streams[audioStreamIndex]->codecpar)) != 0) {
    // Something went wrong. Cleaning up...
  ...
  av_dump_format(formatCtx, audioStreamIndex, NULL, 0);
  ...
  while ((err = av_read_frame(formatCtx, &packet)) != AVERROR_EOF) {
    ...
    if ((err = avcodec_send_packet(codecCtx, &packet)) == 0) {
      ...
      // Something went wrong.
      // EAGAIN is technically no error here but if it occurs we would need to
      // buffer the packet and send it again after receiving more frames. Thus
      // we handle it as an error here.
      printError("Send error.", err);
      break; // Don't return, so we can clean up nicely.
    }

    // Receive and handle frames.
    // EAGAIN means we need to send before receiving again. So thats not an
    // error.
  ...
  // Close the context and free all data associated to it, but not the context
  // itself.
  avcodec_close(codecCtx);
